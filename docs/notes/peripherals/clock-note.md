...
## Cấu hình với ESP-IDF API

Sử dụng hàm `esp_pm_configure()` để thiết lập chế độ hiệu năng cao cho CPU, đảm bảo CPU hoạt động ở tần số tối đa 240MHz.

Tùy chọn cấu hình được lựa chọn là `ESP_PM_CPU_FREQ_MAX` để đảm bảo đạt 240MHz.
